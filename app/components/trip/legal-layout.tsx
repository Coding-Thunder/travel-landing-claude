        ...
        <Container className="py-12 sm:py-16">
          <nav aria-label="Breadcrumb">
            <ol className="flex flex-wrap items-center gap-1.5 text-xs text-navy-500">
              <li><Link href="/" className="hover:text-royal-700">Home</Link></li>
      ...
      <Container className="py-12 sm:py-16">
        ...
        <div className="mt-12 max-w-3xl rounded-2xl border border-navy-100 bg-navy-50 p-6 text-sm text-navy-600 sm:p-8">
          ...
          <p className="mt-1">
            ...
            {hasPhone ? <> · {site.company.phone}</> : null}
          </p>
          <ul className="mt-4 flex flex-wrap gap-x-4 gap-y-1.5 text-xs font-medium text-navy-600">
            {site.legalLinks.map((l) => (
              <li key={l.href}>
                <Link href={l.href} className="underline-offset-4 hover:text-royal-700 hover:underline">{l.label}</Link>
              </li>
            ))}
          </ul>
        </div>
