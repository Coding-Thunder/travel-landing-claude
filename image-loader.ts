 ...
 * the app server re-download each file and re-encode it with sharp, which is
 * the most expensive thing this otherwise static site does, on an instance with
 * one shared vCPU. The optimizer's cache also lives on ephemeral disk, so every
 * deploy paid that cost again.
 *
 ...
 * Responsive srcset still works, because Next calls this per width.
 */
